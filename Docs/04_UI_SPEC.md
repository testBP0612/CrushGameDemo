...
> 基準畫布 1080×1920。**權威版面參考圖：`Art/references/ui_mockup_battle.jpg`**（戰鬥畫面設計目標）。
...
> ⚠ **本表已部分過期（2026-07-07 體檢確認）**：現行實作以 `Scenes/UI/VerticalUi.tscn` 為準——
> HUD 已改為單張 `board.png` 看板、位於 **y330–580**（非下表的 210–390 三欄卡），戰鬥區上緣連帶下移。
> **不要依本表「修正」現行版面**；表列數值僅為歷史設計參考，衝突時一律以場景實況＋二之二不變量為準。
