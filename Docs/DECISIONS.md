...
- **人類決策**（2026-07-11，對話中裁示）：
  ...
  3. **機率 16%**（2026-07-17 人類調整），入 `game_balance.json`（資料驅動，隨時可調）。
...
- **設計師修訂**（2026-07-16，經人類轉達）：網站**配色正本改為
  `Art/art_prompt_style_guide.md`**（美術風格指南：明亮高彩度、粉紅/水藍/
  青綠、藍紫陰影、避免黑暗厚重）；原 `ui_skin.gd` 取色僅保留金色與危險度
  語意色。DESIGN.md 色票升 v2（hex 由 logo/bg 素材取樣），版面不動、只換色。
