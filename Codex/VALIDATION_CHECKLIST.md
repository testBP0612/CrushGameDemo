...
>
> **狀態說明（2026-07-16）**：本檔同時保留可重跑的驗收條目，早期里程碑的空白框未完整回填，
> 不代表功能尚未完成；任務 01–27 的完成狀態以 `AGENTS.md` 與各任務卡完成回報／Git 紀錄為準。
...
- [x] 收益驗證：觸發時本局 `payout_factor` 乘 2、後續倍率持續含 factor、預測/決策/FOMO/結算同源、戰死歸零。
