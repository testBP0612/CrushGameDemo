...
> 講者備註：目標不只是完成一款遊戲，而是驗證「設計＋RD 小團隊＋多個 AI」能以工程化流程穩定出貨。
...
```mermaid
flowchart TD
    A0["人類 × ChatGPT：對話發想玩法與機制"] --> A["人類提出遊戲企劃與比賽目標"]
...
> 講者備註／PPT 重繪建議：三個泳道上色區分——人類（決策節點 A0/A/E/N）、
> Claude（規劃 B/D/F）、Codex＋美術 AI（執行 G–M）。兩個菱形是人類把關點。
> A0＝ChatGPT 發想階段：token 與 Codex／Claude 分開計費，發散討論零開發 token 成本；
> 收斂後才進生產線，對話紀錄在 ChatGPT 端未入 repo（誠實標註）。
...
- 實績：**12 筆範圍與規格疑問全數升級為 Q、經人類裁示後才動工——沒有一筆先斬後奏**（一般 7 筆＋美術 5 筆，均 ANSWERED，`Docs/OPEN_QUESTIONS.md` 可查核）
...
3. 「約三週」口徑＝2026-06-27（Git 最早紀錄）→ 2026-07-16（送件日），人類已確認；現場以「下班時間持續迭代」表述最穩妥。
