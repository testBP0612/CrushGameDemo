...
> 文件狀態：2026-07-16 送件淬鍊版
> 紀錄方式：由 AI 依專案文件、Git、任務卡、決策與驗收紀錄協助彙整；參賽者已實際執行遊戲驗證，並對最終內容負責。  
> 統計口徑：送件簡報採不會因最後整理 commit 失真的保守下限；任務卡與決策數量以 repo 現行文件為準。

## 評審 30 秒摘要

> **不是「AI 幫我做遊戲」，而是把 AI 變成一條有煞車、有證據、有驗收的生產線。**

**27 張任務卡全數完成／25 筆人類決策／12 次 AI 停下提問並由人裁示／1 個可直接玩的 H5 成品。**

先玩成品，再沿著 repo 往回查：每一項功能都能找到規格、決策、任務卡、實作與驗收紀錄。這是本作品最重要的 AI 應用成果，也是與單次 prompt 生成作品的差異。

## 一、專案與 AI 應用摘要

本作品是 Godot 4.6 製作的手機直式 9:16 H5 遊戲。開發目標不只是完成一款遊戲，而是建立一套可複製的 AI 遊戲開發生產線：

> AI 提供槓桿；人類握住方向盤，也握住煞車。每一步都有文件、邊界、決策紀錄與驗收依據。
...
| 項目 | 數量／狀態 | 口徑 |
|---|---:|---|
| Git commits | 120+ | 每一步可追溯；送件採保守下限 |
...
| 已提交 GDScript | 36 份、5,500+ 行 | 可執行成品，不只企劃文件 |
| 開發期間 | 2026-06-27 → 2026-07-16（約三週） | 依 Git 最早紀錄至送件日；為下班時間持續迭代專案 |

## 二、AI 工具使用清單

### 1. ChatGPT（對話型 AI，人類發想階段）

| 項目 | 內容 |
|---|---|
| 主要角色 | 進入生產線之前的發想夥伴：玩法討論、機制取捨、企劃雛形收斂 |
| 實際用途 | 在進入 Claude 規劃前，先與 ChatGPT 來回討論核心循環、下注／過關機制與比賽呈現方向，把發散的想法收斂成可交付 Claude 的企劃輸入 |
| 成本效益 | ChatGPT 與 Codex／Claude 的 token **分開計費**——大量發想來回放在對話型 AI，不消耗開發用 token；進 Claude 規劃時輸入已收斂，規格拆解更有效率 |
| 人類把關 | 發想結論由人類篩選定案後才成為企劃輸入；ChatGPT 不直接觸碰 repo、不寫規格與程式 |
| 代表證據 | 發想對話保存在 ChatGPT 端、未入 repo（誠實標註，由參賽者補登）；收斂後的企劃輸入見 `Planning/00_GAME_PITCH.md` |
...
### 5. Codex ＋ agent-sprite-forge（圖像生成 skill）
...
|---|---|
| 主要角色 | 遊戲 BGM 生成 |
| 實際用途 | 生成主遊戲 BGM（`bgm_main.ogg`）與虎爺事件 BGM（`bgm_huye.ogg`），均已接入並依 `Data/audio.json` 資料驅動播放；缺檔時 fallback 靜音 |
...
```mermaid
flowchart TD
    A0["人類 × ChatGPT：對話發想玩法與機制（token 與開發工具分開計費）"] --> A["人類提出遊戲企劃與比賽目標"]
    A --> B["Claude：拆解規格、資料結構與任務卡"]
    B --> C{"規格是否明確？"}
...
工程先使用 `Assets/placeholders/` 建立完整遊戲循環，美術平行產製正式素材；程式只依賴 `final/` 與 placeholder，缺檔時自動 fallback。這讓兩人小團隊利用下班時間開發時，程式、美術與雲端工作不必互相等待。
...
本專案由設計＋RD 兩人小團隊利用下班時間開發，AI 工具用於規劃、程式、美術協作、雲端操作、驗證與文件整理。遊戲方向、功能取捨、帳號授權、美術採用及最終驗收均由參賽者負責。本文件是依已發生且已由參賽者驗證的專案紀錄補登，並未將未執行的工作寫成完成事項。
