...
> 講者備註：設計＋RD 兩人小團隊利用下班時間開發（2026-06-27 → 2026-07-16，約三週），全程以 repo 文件為人類與多個 AI 工具的共同介面。
...
| AI 工具 | 模型／版本 | 負責範圍 |
|---|---|---|
| ChatGPT | 對話介面（token 與開發工具分開計費） | 人類發想階段：玩法討論、機制取捨、企劃收斂 |
...
## Slide 3：ChatGPT——進生產線之前，先把想法聊清楚

- 定位：**人類發想階段的對話型 AI**——不進 repo、不寫規格、不碰程式
- 用途：核心循環、下注／過關機制、玩法取捨的來回討論與收斂
- 成本：**ChatGPT 與 Codex／Claude 的 token 分開計費**——大量發想對話零開發 token 成本
- 效率：進 Claude 規劃時輸入已收斂，規格拆解更快、來回更少
- **人類把關**：發想結論由人類篩選定案，才成為企劃輸入（`Planning/00_GAME_PITCH.md`）

> 講者備註：發想對話保存在 ChatGPT 端、未入 repo，屬參賽者補登事實（誠實標註）。
> 一句話：把「便宜的發散」留在對話型 AI，把「昂貴的收斂」交給生產線。

---

## Slide 4：Claude／Claude Code——把模糊需求變成規格
...
## Slide 5：OpenAI Codex——把任務卡變成可玩的 H5
...
## Slide 6：美術管線——把 AI 圖變成可直接整合的遊戲資產
...
## Slide 7：Suno AI——讓 AI 音樂進得了正式版本
...
## Slide 8：不是報告：每個數字都有 repo 證據

| 指標 | 數量 |
|---|---:|
| 開發期間 | 2026-06-27 → 2026-07-16，約三週（兩人小團隊、下班時間） |
...
1. 現行送件版為 9 頁（Slide 3 ChatGPT 為 2026-07-16 新增）；若規章另有頁數上限，可合併 6+7、將 9 併入封底，壓成 6–7 頁。
